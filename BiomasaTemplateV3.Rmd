...
<br>a) The Individuals csv file from the Advanced Search,</br>
<br>b) The Wood density file from the query library, </br>
<br>c) The Plot View metadata file from the query library.</br>
...
Once you have installed the programmes and downloaded your data, this template should be ready to run. Clicking the **Knit HTML** button will generate the document. To create a pdf document, select the **Knit PDF** option.
...
#Tbl1 <- gvisTable (Table1, formats= list ('Latitude'='#.###', 'Longitude'= '#.###', 'First census' ='#.##', 'Last census' ='#.##'))
Tbl1<- gvisTable (Table1)
plot(Tbl1, options= list (page='enable',height='automatic', width='automatic'))

```

The plot location is shown in Figure 1.
...
**Results**
Table 2 shows the aboveground biomass estimated using the Chave (2005) moist forest equation with height and wood density as parameters. Height was estimated from tree diameter depending on the region where the plot is located and using the region-specific parameters for a Weibull model of the height/diameter relationship (Feldpaush et al., 2011). 
...
Aboveground biomass change was estimated as the difference in AGB between the first and last census, divided by the time elapsed (Figure 3). In Figure 3, the rate of biomass change over time is indicated by colour and the size of the marker represents the AGB weighted mean.

**Figure 3. Map of Change**

Note: This type of map includes an option for selecting just the region or country of interest, by using the appropriate code in the region option of the gvisGeoChart command. In the configuration options you can find the region codes: link https://developers.google.com/chart/interactive/docs/gallery/geomap
